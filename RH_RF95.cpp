...
HandlerFunc RH_RF95::getIsrHandler(uint8_t num) {
    if (num == 0) {
        return this->isr0;
    } else if (num == 1) {
        return this->isr1;
...
void RH_RF95::handleInterrupt() {
    ...
    // does not actually clear the radio's interrupt flag. So we do it twice.
    // Why?
    spiWrite(RH_RF95_REG_12_IRQ_FLAGS, 0xff);  // Clear all IRQ flags
    spiWrite(RH_RF95_REG_12_IRQ_FLAGS, 0xff);  // Clear all IRQ flags
}
...
void RH_INTERRUPT_ATTR RH_RF95::isr0() {
    if (_deviceForInterrupt[0]) _deviceForInterrupt[0]->handleInterrupt();
}
void RH_INTERRUPT_ATTR RH_RF95::isr1() {
    if (_deviceForInterrupt[1]) _deviceForInterrupt[1]->handleInterrupt();
}
void RH_INTERRUPT_ATTR RH_RF95::isr2() {
    if (_deviceForInterrupt[2]) _deviceForInterrupt[2]->handleInterrupt();
}
...
bool RH_RF95::printRegisters() {
    ...
    return printRegisters(Serial);
#endif
    return true;
}

bool RH_RF95::printRegisters(Print& p) {
    uint8_t registers[] = {
    ...
    for (i = 0; i < sizeof(registers); i++) {
        p.print(registers[i], HEX);
        p.print(": ");
        p.println(spiRead(registers[i]), HEX);
    }
    return true;
}
...
void RH_RF95::setTxPower(int8_t power, bool useRFO) {
    ...
    if (useRFO) {
        ...
        // OutputPower=0xf' RH_RF95_PA_DAC_ENABLE actually adds about 3dBm to
        // all power levels. We will us it for 21, 22 and 23dBm
        if (power > 20) {
        ...
        // PA_BOOST pin is connected, so must use PA_BOOST Pout = 2 +
        // OutputPower. The documentation is pretty confusing on this topic:
        // PaSelect says the max power is 20dBm, but OutputPower claims it would
        // be 17dBm. My measurements show 20dBm is correct
...
void RH_RF95::setLowDatarate() {
    ...
    // "To avoid issues surrounding  drift  of  the  crystal  reference
    // oscillator  due  to  either  temperature  change or  motion,the  low data
    // rate optimization  bit  is  used. Specifically for 125  kHz  bandwidth
    // and  SF  =  11  and  12, this  adds  a  small  overhead  to increase
    // robustness to reference frequency variations over the timescale of the
    // LoRa packet."

    // read current value for BW and SF
    uint8_t BW =
        spiRead(RH_RF95_REG_1D_MODEM_CONFIG1) >> 4;  // bw is in bits 7..4
    uint8_t SF =
        spiRead(RH_RF95_REG_1E_MODEM_CONFIG2) >> 4;  // sf is in bits 7..4
