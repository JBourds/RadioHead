...
/// LoRa capable radio transceiver.
///
/// For Semtech SX1276/77/78/79 and HopeRF RF95/96/97/98 and other similar LoRa
/// capable radios. Based on http://www.hoperf.com/upload/rf/RFM95_96_97_98W.pdf
...
///
/// Works with
/// - the excellent MiniWirelessLoRa from Anarduino
...
/// - The very fine Talk2 Whisper Node LoRa boards
/// https://wisen.com.au/store/products/whisper-node-lora
///   an Arduino compatible board, which include an on-board RFM95/96 LoRa Radio
///   (Semtech SX1276), external antenna, run on 2xAAA batteries and support low
///   power operations. RF95 examples work without modification. Use Arduino
///   Board Manager to install the Talk2 code support. Upload the code with an
///   FTDI adapter set to 5V.
...
///
/// Manager classes may use this class to implement reliable, addressed
/// datagrams and streams, mesh routers, repeaters, translators etc.
///
/// Naturally, for any 2 radios to communicate that must be configured to use
/// the same frequency and modulation scheme.
///
...
/// compatible radio modules in LoRa mode.
///
/// The Hope-RF (http://www.hoperf.com) RFM95/96/97/98(W) and Semtech
/// SX1276/77/78/79 is a low-cost ISM transceiver chip. It supports FSK, GFSK,
/// OOK over a wide range of frequencies and programmable data rates, and it
/// also supports the proprietary LoRA (Long Range) mode, which is the only mode
/// supported in this RadioHead driver.
///
...
/// predefined Bandwidths, Spreading Factors and Coding Rates.  Frequency can be
/// set with 61Hz precision to any frequency from 240.0MHz to 960.0MHz. Caution:
/// most modules only support a more limited range of frequencies due to antenna
/// tuning.
///
...
/// on OpenSuSE 13.1.
/// Also tested with Teensy3.1, Modtronix inAir4 and Arduino 1.6.5 on
/// OpenSuSE 13.1
///
/// \par Packet Format
///
/// All messages sent and received by this RH_RF95 Driver conform to this packet
/// format:
///
/// - LoRa mode:
/// - 8 symbol PREAMBLE
...
///
/// We tested with Anarduino MiniWirelessLoRA, which is an Arduino Duemilanove
/// compatible with a RFM96W module on-board. Therefore it needs no connections
/// other than the USB programming connection and an antenna to make it work.
///
/// If you have a bare RFM95/96/97/98 that you want to connect to an Arduino,
/// you might use these connections (untested): CAUTION: you must use a 3.3V
/// type Arduino, otherwise you will also need voltage level shifters between
/// the Arduino and the RFM95.  CAUTION, you must also ensure you connect an
/// antenna.
///
/// \code
...
/// for interrupts).
///
/// You can connect a Modtronix inAir4 or inAir9 directly to a 3.3V part such as
/// a Teensy 3.1 like this (tested).
/// \code
...
///
/// Note that if you are using Modtronix inAir4 or inAir9,or any other module
/// which uses the transmitter RFO pins and not the PA_BOOST pins that you must
/// configure the power transmitter power for -1 to 14 dBm and with useRFO true.
...
/// (Pin 2 is for the NMI only). The same comments apply to Pin 4 on Arduino
/// Zero from arduino.cc. Instead you can use any other pin (we use Pin 3) and
/// initialise RH_RF69 like this:
/// \code
...
/// - Ensure you have Arduino SAMD board support 1.6.5 or later in Arduino
/// IDE 1.6.8 or later.
...
/// RH_RF95 driver(5, 2);
/// \endcode
/// - The name of the serial port on that board is 'SerialUSB', not 'Serial', so
/// this may be helpful at the top of our
///   sample sketches:
/// \code
...
/// \code
/// // Ensure serial flash is not interfering with radio communication on SPI
/// bus
///  pinMode(4, OUTPUT);
///  digitalWrite(4, HIGH);
/// \endcode
/// - and if you have a 915MHz part, you need this after driver/manager
/// intitalisation:
/// \code
...
/// {
///   // Ensure serial flash is not interfering with radio communication on SPI
///   bus pinMode(4, OUTPUT); digitalWrite(4, HIGH);
///
///   Serial.begin(9600);
...
/// the example rf95_* sketches work without modification. Initialise the radio
/// like with the default constructor:
/// \code
///  RH_RF95 driver;
/// \endcode
///
...
/// master mode.
///
/// Caution: Power supply requirements of the RFM module may be relevant in some
/// circumstances: RFM95/96/97/98 modules are capable of pulling 120mA+ at full
/// power, where Arduino's 3.3V line can give 50mA. You may need to make
/// provision for alternate power supply for the RFM module, especially if you
/// wish to use full transmit power, and/or you have other shields demanding
/// power. Inadequate power for the RFM is likely to cause symptoms such as:
/// - reset's/bootups terminate with "init failed" messages
/// - random termination of communication after 5-30 packets sent/received
/// - "fake ok" state, where initialization passes fluently, but communication
/// doesn't happen
...
///
/// It is often hard to accurately identify when you are hitting RAM limits on
/// Arduino. The symptoms can include:
/// - Mysterious crashes and restarts
/// - Changes in behaviour when seemingly unrelated changes are made (such as
...
/// - rf95_client base station connected to a VHF discone antenna at 8m height
/// above ground
/// - rf95_server mobile connected to 17.3cm 1/4 wavelength antenna at 1m
/// height, no ground plane.
...
///
/// It should be noted that at this data rate, a 12 octet message takes 2
/// seconds to transmit.
///
...
/// - Range over flat ground through heavy trees and vegetation approx 2km.
///
/// Caution: the performance of this radio, especially with narrow bandwidths is
/// strongly dependent on the accuracy and stability of the chip clock. HopeRF
/// and Semtech do not appear to recommend bandwidths of less than 62.5 kHz
/// unless you have the optional Temperature Compensated Crystal Oscillator
/// (TCXO) installed and enabled on your radio module. See the refernece manual
/// for more data. Also
/// https://lowpowerlab.com/forum/rf-range-antennas-rfm69-library/lora-library-experiences-range/15/
/// and
...
/// programmed power for Anarduino MiniWirelessLoRa (which has RFM96W-433Mhz
/// installed)
...
/// - 12db attenuator
/// - SMA connector
/// - MiniKits AD8307 HF/VHF Power Head (calibrated against Rohde&Schwartz
/// 806.2020 test set)
...
/// \endcode
///
/// We have also measured the actual power output from a Modtronix inAir4
/// http://modtronix.com/inair4.html connected to a Teensy 3.1: Teensy 3.1 this
/// is a 3.3V part, connected directly to: Modtronix inAir4 with SMA antenna
/// connector, connected as above: 10cm SMA-SMA cable
/// - MiniKits AD8307 HF/VHF Power Head (calibrated against Rohde&Schwartz
/// 806.2020 test set)
/// - Tektronix TDS220 scope to measure the Vout from power head
/// \code
...
/// You would not expect to get anywhere near these powers to air with a simple
/// 1/4 wavelength wire antenna.
class RH_RF95 : public RHSPIDriver {
   public:
    /// \brief Defines register values for a set of modem configuration
    /// registers
    ///
    ...
    /// desired settings. It might be helpful to use the LoRa calculator
    /// mentioned in
    /// http://www.semtech.com/images/datasheet/LoraDesignGuide_STD.pdf
    /// These are indexes into MODEM_CONFIG_TABLE. We strongly recommend you use
    /// these symbolic definitions and not their integer equivalents: its
    /// possible that new values will be introduced in later versions (though we
    /// will try to avoid it). Caution: if you are using slow packet rates and
    /// long packets with RHReliableDatagram or subclasses you may need to
    /// change the RHReliableDatagram timeout for reliable operations. Caution:
    /// for some slow rates nad with ReliableDatagrams youi may need to increase
    /// the reply timeout with manager.setTimeout() to deal with the long
    /// transmission times.
    typedef enum {
    ...
    /// have its own interrupt and slave select pin. After constructing, you
    /// must call init() to initialise the interface and the radio module. A
    /// maximum of 3 instances can co-exist on one processor, provided there are
    /// sufficient distinct interrupt lines, one for each instance.
    /// \param[in] slaveSelectPin the Arduino pin number of the output to use to
    /// select the RH_RF22 before accessing it. Defaults to the normal SS pin
    /// for your Arduino (D10 for Diecimila, Uno etc, D53 for Mega, D10 for
    /// Maple)
    /// \param[in] interruptPin The interrupt Pin number that is connected to
    /// the RFM DIO0 interrupt line. Defaults to pin 2, as required by Anarduino
    /// MinWirelessLoRa module. Caution: You must specify an interrupt capable
    /// pin. On many Arduino boards, there are limitations as to which pins may
    /// be used as interrupts. On Leonardo pins 0, 1, 2 or 3. On Mega2560 pins
    /// 2, 3, 18, 19, 20, 21. On Due and Teensy, any digital pin. On Arduino
    /// Zero from arduino.cc, any digital pin other than 4. On Arduino M0 Pro
    /// from arduino.org, any digital pin other than 2. On other Arduinos pins 2
    /// or 3. See http://arduino.cc/en/Reference/attachInterrupt for more
    /// details. On Chipkit Uno32, pins 38, 2, 7, 8, 35. On other boards, any
    /// digital pin may be used.
    ...
    /// to the Serial device if RH_HAVE_SERIAL is defined for the current
    /// platform For debugging purposes only.
    ...
    /// Sets all the registered required to configure the data modem in the
    /// RF95/96/97/98, including the bandwidth, spreading factor etc. You can
    /// use this to configure the modem with custom configurations if none of
    /// the canned configurations in ModemConfigChoice suit you.
    /// \param[in] config A ModemConfig structure containing values for the
    ...
    /// ModemConfig. Caution: the slowest protocols may require a radio module
    /// with TCXO temperature controlled oscillator for reliable operation.
    ...
    /// \return true if a new, complete, error-free uncollected message is
    /// available to be retreived by recv()
    ...
    /// frequently enough to not miss any messages It is recommended that you
    ...
    /// Detection (CAD) to show the channnel is clear (if the radio supports
    /// CAD) by calling waitCAD(). Then loads a message into the transmitter and
    /// starts the transmitter. Note that a message length of 0 is permitted.
    /// \param[in] data Array of data to be sent
    /// \param[in] len Number of bytes of data to send
    /// specify the maximum time in ms to wait. If 0 (the default) do not wait
    /// for CAD before transmitting.
    /// \return true if the message length was valid and it was correctly queued
    /// for transmit. Return false if CAD was requested and the CAD timeout
    /// timed out before clear channel was detected.
    ...
    /// centre frequency.
    /// \param[in] centre Frequency in MHz. 137.0 to 1020.0. Caution:
    /// RFM95/96/97/98 comes in several different frequency ranges, and setting
    /// a frequency outside that range of your radio will probably not work
    /// \return true if the selected frquency centre is within range
    bool setFrequency(float centre);

    /// If current mode is Rx or Tx changes it to Idle. If the transmitter or
    /// receiver is running, disables them.
    void setModeIdle();
    ...
    /// Sets the transmitter power output level, and configures the transmitter
    /// pin. Be a good neighbour and set the lowest power level you need. Some
    /// SX1276/77/78/79 and compatible modules (such as RFM95/96/97/98) use the
    /// PA_BOOST transmitter pin for high power output (and optionally the
    /// PA_DAC) while some (such as the Modtronix inAir4 and inAir9) use the RFO
    /// transmitter pin for lower power but higher efficiency. You must set the
    /// appropriate power level and useRFO argument for your module. Check with
    /// your module manufacturer which transmtter pin is used on your module to
    /// ensure you are setting useRFO correctly. Failure to do so will result in
    /// very low transmitter power output. Caution: legal power limits may apply
    /// in certain countries. After init(), the power will be set to 13dBm, with
    /// useRFO false (ie PA_BOOST enabled).
    /// \param[in] power Transmitter power level in dBm. For RFM95/96/97/98 LORA
    /// with useRFO false, valid values are from +5 to +23. For Modtronix inAir4
    /// and inAir9 with useRFO true (ie RFO pins in use), valid values are from
    /// -1 to 14.
    /// \param[in] useRFO If true, enables the use of the RFO transmitter pins
    /// instead of the PA_BOOST pin (false). Choose the correct setting for your
    /// module.
    void setTxPower(int8_t power, bool useRFO = false);

    /// Sets the radio into low-power sleep mode.
    /// If successful, the transport will stay in sleep mode until woken by
    /// changing mode it idle, transmit or receive (eg by calling send(),
    /// recv(), available() etc) Caution: there is a time penalty as the radio
    /// takes a finite time to wake from sleep mode.
    /// \return true if sleep mode was successfully entered.
    virtual bool sleep();

    // Bent G Christensen (bentor@gmail.com), 08/15/2016
    /// Use the radio's Channel Activity Detect (CAD) function to detect channel
    /// activity. Sets the RF95 radio into CAD mode and waits until CAD
    /// detection is complete. To be used in a listen-before-talk mechanism
    /// (Collision Avoidance) with a reasonable time backoff algorithm. This is
    /// called automatically by waitCAD().
    /// \return true if channel is in use.
    virtual bool isChannelActive();

    /// Enable TCXO mode
    /// Call this immediately after init(), to force your radio to use an
    /// external frequency source, such as a Temperature Compensated Crystal
    /// Oscillator (TCXO), if available. See the comments in the main
    /// documentation about the sensitivity of this radio to clock frequency
    /// especially when using narrow bandwidths. Leaves the module in sleep
    /// mode. Caution, this function has not been tested by us. Caution, the
    /// TCXO model radios are not low power when in sleep (consuming about ~600
    /// uA, reported by Phang Moh Lim.<br>
    void enableTCXO();
    ...
    /// Caution: this measurement is not absolute, but is measured relative to
    /// the local receiver's oscillator. Apparent errors may be due to the
    /// transmitter, the receiver or both.
    /// \return The estimated centre frequency offset in Hz of the last received
    /// message. If the modem bandwidth selector in register
    /// RH_RF95_REG_1D_MODEM_CONFIG1 is invalid, returns 0.
    int frequencyError();
    ...
    /// 12501-250000 250.0
    /// >250000      500.0
    /// NOTE caution Earlier - Semtech do not recommend BW below 62.5 although,
    /// in testing I managed 31.25 with two devices in close proximity.
    /// \param[in] sbw long, signal bandwidth e.g. 125000
    void setSignalBandwidth(long sbw);

    /// brian.n.norman@gmail.com 9th Nov 2018
    /// Sets the coding rate to 4/5, 4/6, 4/7 or 4/8.
    /// Valid denominator values are 5, 6, 7 or 8. A value of 5 sets the coding
    /// rate to 4/5 etc. Values below 5 are clamped at 5 values above 8 are
    /// clamped at 8
    ...
    /// called by setBandwidth() and setSpreadingfactor() since these affect the
    /// symbol time.
    void setLowDatarate();

    /// brian.n.norman@gmail.com 9th Nov 2018
    /// allows the payload CRC bit to be turned on/off. Normally this should be
    /// left on so that packets with a bad CRC are rejected
    /// \patam[in] on bool, true turns the payload CRC on, false turns it off
    void setPayloadCRC(bool on);

    HandlerFunc getIsrHandler(uint8_t num);

   protected:
    /// This is a low level function to handle the interrupts for one instance
    /// of RH_RF95. Called automatically by isr*() Should not need to be called
    /// by user code.
    void handleInterrupt();

    /// Examine the revceive buffer to determine whether the message is for this
    /// node
    void validateRxBuf();
